...
export default class TestProps extends Component{
    ...
    // 它表示，在真实DOM上将要渲染的视图结构
    render(){
        //在这里可以做一些事
        let{msg , buley , bol , arrs ,user} = this.props
        let{arr} = this.state;//es6语法

        let num = 0;
        for(let i=0;i<6;i++){//for循环
            num += arr[i]
        } 
        ...
        return(
            <div>
                ...
                {
                    bol&&arrs.map((ele,idx)=><div key={idx}>{ele*10000}</div>)
                }
                {
