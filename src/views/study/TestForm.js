...
// 受控表单：表单的 value/checked 由state 完全控制
...
export default class TestForm extends React.Component{
    ...
    change(e,key){
        console.log('选择',e.target.value,e.target.checked,key);
        switch(key){
            // case 'name':
            //     this.setState(state=>({
            //         name:e.target.value
    ...
    render(){
        ...
        return (
            <div>
                ...
                <input ref='pass' type='password'/><br/>{/* this.ref.value */}
                <input type='text' onInput={(e)=>this.getAge(e)}/><br/>
                { /*在React中，有且仅有一个你可以放心使用的非受控制表单 */ }
                <input type='file'/><br/>
                <button onClick={()=>this.submitUnctrlForm()}>提交非受控表单</button><br/>
                ...
                <span>选择你的爱好：</span>
                  ...
                  <br/>
                  <button onClick={()=>this.submitCtrlForm()}>提交表单</button>
            </div>
        )
    }
}